...
function GoogleSSOButton({ variant = "light", onClick }) {
  const googlesignup = () => {
    ...
    const redirectUrl = import.meta.env.VITE_API_URL; // Correct usage
    window.location.href = `${redirectUrl}/api/v1/users/google`;
  };
  ...
  return (
    <div className="flex items-center justify-center">
      <button
        ...
        className={`flex items-center justify-center gap-2 px-6 py-2 border rounded-full transition-colors ${
          variant === "dark"
            ? "bg-[#1a1a1a] hover:bg-[#2a2a2a] border-[#1a1a1a] text-white"
